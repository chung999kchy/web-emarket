...
<div id="container">
        ...
        <div id="prod_nav">
            <ul>
                ...
                <c:forEach var="product" items="${newProducts}">
                    <li>
                        <a href="#${countProduct}">
                            <img src="${initParam.imageProductPath}${product.getThumbImage()}" alt="">
    ...
    <c:forEach var="category" items="${category}">
        <c:if test="${category.categoryId == 4}">
            <div class="one-fourth last" style="margin: 20px 10px">
                <div class="heading_bg">
                    <h2>${category.name}</h2>
                </div>
                <a href="<c:url value='category?${category.categoryId}'/>">
                    <img src="${initParam.imgProductPath}${category.getImage()}" alt="" width="217px" height="200px">
                </a>
            </div>
        </c:if>
        <c:if test="${category.categoryId < 4}">
            <div class="one-fourth" style="margin: 20px 10px">
                <div class="heading_bg">
                    <h2>${category.name}</h2>
                </div>
                <a href="<c:url value='category?${category.categoryId}'/>">
                    <img src="${initParam.imgProductPath}${category.getImage()}" alt="" width="217px;" height="150px">
                </a>
            </div>
        </c:if>
    </c:forEach>
